...
class MyLogger:
    def __init__(self):
        ...
        # 清空所有设置
        self.logger.remove()
        # 添加控制台输出的格式，sys.stdout 为输出到屏幕；关于这些配置还需要自定义请移到官网查看相关参数说明
        ...
        #                         "{process.name} | "    #进程名
        #                         "{thread.name} | "    #线程名
        #                         '{moudle}.{function}:{line} - {level} -{message}',  # 模块名.方法名：行号
        #                 rotation="10 MB",   # 日志生成的规则 rotation=“1 week“ rotation=“1 days“
